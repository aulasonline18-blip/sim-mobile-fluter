...
void main() {
  test('organismo ideal nasce com todos os orgaos vivos conectados', () {
    ...
    expect(organism.health.serverOnlyOrgans, contains('/api/generate-lesson-image'));
    expect(organism.health.serverOnlyOrgans, contains('/api/generate-lesson-audio'));
    expect(organism.health.serverOnlyOrgans, contains('/api/public/payments/webhook'));
  });

  test('roteador protege ambientes que precisam de identificacao, idioma e objetivo', () {
  ...
  test('fluxo vivo vai de login para idioma, objetivo, preparo e nivelamento/aula', () async {
    final organism = SimOrganism.laboratory();
  ...
  test('sync, creditos e portas externas permanecem disponiveis sem segredo no app', () async {
    final organism = SimOrganism.laboratory();
