...
const _messageKeys = [
  'prep_msg_1',  'prep_msg_2',  'prep_msg_3',  'prep_msg_4',
  'prep_msg_5',  'prep_msg_6',  'prep_msg_7',  'prep_msg_8',
  'prep_msg_9',  'prep_msg_10', 'prep_msg_11', 'prep_msg_12',
];
...
class _SimPreparationExperienceState extends State<SimPreparationExperience>
  ...
  void initState() {
    ...
    _fadeInCtrl = AnimationController(
      ...
      ..forward();
    _bobCtrl    = AnimationController(vsync: this, duration: const Duration(milliseconds: 3400))..repeat(reverse: true);
    _nodCtrl    = AnimationController(vsync: this, duration: const Duration(milliseconds: 4000))..repeat(reverse: true);
    _waveRCtrl  = AnimationController(vsync: this, duration: const Duration(milliseconds: 2600))..repeat(reverse: true);
    _waveLCtrl  = AnimationController(vsync: this, duration: const Duration(milliseconds: 3100))..repeat(reverse: true);
    _sparkCtrl  = AnimationController(vsync: this, duration: const Duration(milliseconds: 2000))..repeat(reverse: true);
    _glowCtrl   = AnimationController(vsync: this, duration: const Duration(milliseconds: 5000))..repeat(reverse: true);
    _floatCtrl  = AnimationController(vsync: this, duration: const Duration(milliseconds: 9000))..repeat();
  ...
  void _updateMessageKeys() {
    ...
    _activeKeys = s == 'review'       ? _reviewPrepKeys
               : s == 'reviewDone'   ? _reviewDoneKeys
               : s == 'recovery'     ? _recoveryPrepKeys
               : s == 'recoveryDone' ? _recoveryDoneKeys
               : isDone              ? _doneMessageKeys
               :                       _messageKeys;
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isDone  = stage == 'done' || stage == 'reviewDone' || stage == 'recoveryDone';
    ...
    final btnLabel = ready
        ? (stage == 'review'       ? t('aux_review_start_cta')
           : stage == 'reviewDone'   ? t('aux_review_continue_cta')
           : stage == 'recovery'     ? t('aux_recovery_start_cta')
           : stage == 'recoveryDone' ? t('aux_recovery_finish_cta')
           : isDone                  ? t('done_cta')
           :                           t('continue_arrow'))
        : t('preparing_short');
                            ...
                            child: Container(
                              decoration: const BoxDecoration(
                                gradient: RadialGradient(
                                  colors: [Color(0x24111827), Color(0x00111827)],
                                  radius: 0.7,
                                ),
                              ),
                            ),
                              ...
                              child: SizedBox(
                                ...
                                child: CustomPaint(
                                  painter: _RobotPainter(
                                    armLeftAngle:  (waveL * 2 - 1) * 18 * pi / 180,
                                    armRightAngle: (waveR * 2 - 1) * 22 * pi / 180,
...
class _RobotPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Shadow
    fill.color = const Color(0xFF0F172A).withOpacity(0.10);
    canvas.drawOval(
    ...
    final bodyRRect = RRect.fromRectAndRadius(
      const Rect.fromLTWH(58, 104, 104, 104), const Radius.circular(30));
  ...
  void _drawArm(Canvas canvas, Paint fill, Paint stroke,
      double rx, double ry, double cx, double cy) {
    fill.color = Colors.white;
    stroke.color = _simDark;
    stroke.strokeWidth = 3;
    final rr = RRect.fromRectAndRadius(
        Rect.fromLTWH(rx, ry, 16, 42), const Radius.circular(8));
  ...
  void _drawHead(Canvas canvas, Paint fill, Paint stroke) {
    ...
    stroke.color = _simDark.withOpacity(sparkOpacity);
      ...
      child: SizedBox.expand(
        child: Stack(
          children: [
            ...
            _FloatIcon(
              top: 0.10, leftFraction: 0.05,
              dy: _floatY(delays[0]), rot: _floatRot(delays[0]),
              child: _IconBook(),
            ),
            // Icon 2: card — top-right
            _FloatIcon(
              top: 0.16, rightFraction: 0.06,
              dy: _floatY(delays[1]), rot: _floatRot(delays[1]),
              child: _IconCard(),
            ),
            // Icon 3: path — middle-left
            _FloatIcon(
              bottomFraction: 0.34, leftFraction: 0.03,
              dy: _floatY(delays[2]), rot: _floatRot(delays[2]),
              child: _IconPath(),
            ),
            // Icon 4: spark — middle-right
            _FloatIcon(
              top: 0.44, rightFraction: 0.04,
              dy: _floatY(delays[3]), rot: _floatRot(delays[3]),
              child: _IconSpark(),
            ),
            // Icon 5: dot — lower-right
            _FloatIcon(
              bottomFraction: 0.24, rightFraction: 0.18,
              dy: _floatY(delays[4]), rot: _floatRot(delays[4]),
              child: _IconDot(),
            ),
            // Icon 6: dot — lower-left
            _FloatIcon(
              bottomFraction: 0.14, leftFraction: 0.22,
              dy: _floatY(delays[5]), rot: _floatRot(delays[5]),
              child: _IconDot(),
            ),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(builder: (_, constraints) {
      ...
      return Stack(children: [
        Positioned(
          top: tp, bottom: bt, left: l, right: r,
          child: Transform(
...
class _IconBook extends StatelessWidget {
  ...
  Widget build(BuildContext context) => CustomPaint(
    ...
    painter: _IconPainter(_paintBook),
  );

  static void _paintBook(Canvas c, Size s) {
    final f = Paint()..color = const Color(0xFFD1D5DB).withOpacity(0.85)..style = PaintingStyle.fill;
    final st = Paint()..color = const Color(0xFF374151)..style = PaintingStyle.stroke..strokeWidth = 1.5;
...
class _IconCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => CustomPaint(
    ...
    painter: _IconPainter(_paintCard),
  );

  static void _paintCard(Canvas c, Size s) {
    final f = Paint()..color = Colors.white..style = PaintingStyle.fill;
    final st = Paint()..color = const Color(0xFF374151)..style = PaintingStyle.stroke..strokeWidth = 1.5;
    final l1 = Paint()..color = const Color(0xFF6B7280)..style = PaintingStyle.stroke..strokeWidth = 2..strokeCap = StrokeCap.round;
...
class _IconPath extends StatelessWidget {
  ...
  Widget build(BuildContext context) => CustomPaint(
    size: const Size(38, 38),
    painter: _IconPainter(_paintPath),
  );

  static void _paintPath(Canvas c, Size s) {
    final st = Paint()..color = const Color(0xFF374151)..style = PaintingStyle.stroke..strokeWidth = 2.2..strokeCap = StrokeCap.round;
    final f = Paint()..color = const Color(0xFF111827)..style = PaintingStyle.fill;
    final p = Path()..moveTo(4, 24)..quadraticBezierTo(12, 8, 28, 16);
    c.drawPath(p, st);
...
class _IconSpark extends StatelessWidget {
  ...
  Widget build(BuildContext context) => CustomPaint(
    ...
    painter: _IconPainter(_paintSpark),
  );

  static void _paintSpark(Canvas c, Size s) {
    final f = Paint()..color = const Color(0xFFD1D5DB)..style = PaintingStyle.fill;
    final st = Paint()..color = const Color(0xFF374151)..style = PaintingStyle.stroke..strokeWidth = 1.2..strokeJoin = StrokeJoin.round;
    final p = Path()
      ..moveTo(16, 4)..lineTo(18, 14)..lineTo(28, 16)
      ..lineTo(18, 18)..lineTo(16, 28)..lineTo(14, 18)
      ..lineTo(4, 16)..lineTo(14, 14)..close();
    c.drawPath(p, f);
    c.drawPath(p, st);
  }
}
...
class _IconDot extends StatelessWidget {
  ...
  Widget build(BuildContext context) => CustomPaint(
    ...
    painter: _IconPainter(_paintDot),
  );

  static void _paintDot(Canvas c, Size s) {
    c.drawCircle(const Offset(8, 8), 6,
        Paint()..color = const Color(0xFF9CA3AF).withOpacity(0.7));
  }
}
    ...
    Widget btn = SizedBox(
      ...
      child: DecoratedBox(
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: ready ? _simBorder : Colors.transparent,
          ),
          boxShadow: ready
              ? const [
                  BoxShadow(
