...
String? sanitizeAndEncodeSvg(Object? raw) {
  ...
  if (lower.contains('<script')) return null;
  if (RegExp(r'\son[a-z]+\s*=').hasMatch(lower)) return null;
  if (RegExp(r'javascript\s*:', caseSensitive: false).hasMatch(svg)) return null;
  if (RegExp(r'<foreignobject', caseSensitive: false).hasMatch(lower)) return null;
...
VisualDecision decideVisualGeneration(
  ...
  if (vt == null || vt is! Map) {
    return const VisualDecision(generate: false, prompt: null, svg: null, reason: 'S12_NO_VISUAL_TRIGGER');
  }
  if (vt['needs_image'] != true) {
    return const VisualDecision(generate: false, prompt: null, svg: null, reason: 'S12_NEEDS_IMAGE_FALSE');
  }
  if (vt['pedagogical_need'] == 'none') {
    return const VisualDecision(generate: false, prompt: null, svg: null, reason: 'S12_PEDAGOGICAL_NEED_NONE');
  }
  ...
  if (vt['render_strategy'] == 'software') {
    ...
    if (svg != null) {
      return VisualDecision(generate: false, prompt: null, svg: svg, reason: 'S12_SVG_INLINE_OK');
    }
    final softwarePrompt = vt['image_prompt']?.toString().trim() ?? '';
    final softwareTopic = vt['topic']?.toString().trim() ?? '';
    if (softwarePrompt.isNotEmpty || softwareTopic.isNotEmpty || vt['math_template'] != null) {
  ...
  if (contexto.priority != 'active') {
    return const VisualDecision(generate: false, prompt: null, svg: null, reason: 'S12_BLOCK_PREFETCH_BACKGROUND');
  }
  if (!contexto.allowPaidImages) {
    return const VisualDecision(generate: false, prompt: null, svg: null, reason: 'S12_PAID_IMAGES_DISABLED');
  }

  final imagePrompt = vt['image_prompt']?.toString().trim() ?? '';
  if (imagePrompt.isNotEmpty) {
    return VisualDecision(generate: true, prompt: imagePrompt, svg: null, reason: 'S12_OK_IMAGE_PROMPT');
  }

  final topic = vt['topic']?.toString().trim() ?? '';
  if (topic.isNotEmpty) {
    final lang = contexto.stableLang?.trim() ?? 'the student\'s selected language';
    final prompt =
    ...
    return VisualDecision(generate: true, prompt: prompt, svg: null, reason: 'S12_OK_TOPIC_ONLY');
  }

  return const VisualDecision(generate: false, prompt: null, svg: null, reason: 'S12_NO_PROMPT_NO_TOPIC');
}
